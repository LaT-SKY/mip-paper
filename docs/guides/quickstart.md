...
`interactionEnabled: false` 时壁纸窗口整体穿透鼠标，右键菜单与设置入口均不可用；此时仍可通过编辑 `~/.config/mip-paper/config.json` 恢复。
...
**聚焦自动消失**：`menu.closeOnFocusChange`（默认 `true`）开启后，KWin 协调器在任意非壁纸窗口获得焦点时通知主进程，各显示器上的菜单自动收起。同时，指针移出壁纸窗口也会立即收起菜单——这覆盖了点击**已经聚焦**的窗口的情况。壁纸窗口本身忽略焦点，因此右键打开菜单不会误触关闭。菜单自身与应用的 GUI 窗口（如设置窗口）不会被这些机制收起。
