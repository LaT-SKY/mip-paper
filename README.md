...
**聚焦自动消失**：`menu.closeOnFocusChange`（默认 `true`）开启后，KWin 协调器在任意非壁纸窗口获得焦点（例如点击了其他应用）时通知主进程，各显示器上的菜单自动收起。同时，指针移出壁纸窗口（移向其他窗口或面板）也会立即收起菜单——这覆盖了点击**已经聚焦**的窗口的情况（焦点未变化时没有激活信号，但指针必然离开了壁纸）。壁纸窗口本身忽略焦点，因此右键打开菜单不会误触关闭。

**自身界面排除**：菜单自身是叠在画布上的独立表面，指针移到菜单上不会收起（在菜单项之间移动选择不会误关）；后续随应用一起开发的 GUI 窗口（如设置对话框）也不会触发收起——它们是同一应用（`mip-paper` 窗口类）的一部分，聚焦它们或把指针移入它们都不算“聚焦其他应用”，KWin 协调器与主进程都会将其排除在自动关闭之外。

作为兜底，`menu.autoCloseMs` 可设置菜单打开后无人操作自动关闭的毫秒数（`0` 表示关闭该兜底），适合协调器不可用或希望菜单绝不残留的场景；三个机制独立生效。
