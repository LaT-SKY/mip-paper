...
export function validateRuntimeConfig(value) {
  const valid = object(value)
    ...
    && (value.audio.style === undefined || ['ribbon','bars','wave','mirror'].includes(value.audio.style))
    ...
    && (value.audio.barCount === undefined || (Number.isInteger(value.audio.barCount) && value.audio.barCount >=16 && value.audio.barCount <=72))
    && (value.audio.mirrored === undefined || typeof value.audio.mirrored === 'boolean')
