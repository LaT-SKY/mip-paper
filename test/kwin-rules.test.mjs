...
test('install adds an exact project rule without replacing existing rules', async () => {
  ...
  try {
    ...
    assert.equal(await readKey(file, 'animated-ocean-wallpaper', 'fullscreen', 'missing'), 'missing');
    assert.equal(await readKey(file, 'animated-ocean-wallpaper', 'fullscreenrule', 'missing'), 'missing');
