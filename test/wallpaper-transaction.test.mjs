...
function transaction(name, { key, identity, generation, analyze = true, rgb = [255, 52, 120] }) {
  return {
...
test('promotes and analyzes only the latest transaction when decodes finish out of order', async () => {
  ...
  decodes.get('file:///wallpapers/b').resolve({ name: 'b', rgb: [220, 90, 70] });
  await pendingB;
  decodes.get('file:///wallpapers/a').resolve({ name: 'a', rgb: [31, 173, 158] });
  await pendingA;
...
test('does not submit analysis that becomes stale while analysis is pending', async () => {
  ...
  const coordinator = createWallpaperTransactionCoordinator({
    ...
    analyzeImage: (image) => image.url.endsWith('/a') ? analysisA.promise : [220, 90, 70],
    submitAccent: async (submission) => { submissions.push(submission); },
    applyColor: () => {},
    promoteImage: () => {},
  });
...
test('applies a cached color with image promotion and skips analysis', async () => {
  ...
  const coordinator = createWallpaperTransactionCoordinator({
    ...
    applyColor: (color) => events.push(['color', color.rgb]),
    promoteImage: (image) => events.push(['image', image.name]),
  });
  await coordinator.apply(transaction('a', {
    key: keyA, identity: identityA, generation: 3, analyze: false, rgb: [31, 173, 158],
  }));
  assert.deepEqual(events, [['image', 'a'], ['color', [31, 173, 158]]]);
});
...
test('submits the immutable identity, content key, and generation from the analyzed transaction', async () => {
  ...
  assert.deepEqual(submissions, [{
    rgb: [31, 173, 158], wallpaperIdentity: identityA, contentKey: keyA, generation: 7,
  }]);
});
